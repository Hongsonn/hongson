...
DEBUG_FILE="/dev/null"
#DEBUG_FILE="/home/bachnt/debug.log"
#############################################
# Argument to pass to the script and its manipulation
#############################################
echo test > /home/bachnt/debug.log
USER=$1
SUBJECT=$2
DEBUG_SUBJECT=$2
TEXT=$3
...
# Zabbix address
#############################################
ZBX_URL="http://192.168.15.3/zabbix"
...
if [ $(($SEND_GRAPH)) -eq '1' ]; then
	...
	echo "zzz"
	echo ${CURL} -k -s -S --max-time 5 -X POST "https://api.telegram.org/bot${BOT_TOKEN}/sendPhoto" -F chat_id=${USER} -F photo="@${PNG_PATH}" -F caption="${TEXT}" > /home/bachnt/debug.log
fi
